...
def numberTripsStation(
    station_id, station_activity
) -> int:  # número de viajes realizados en una estación
...
def peakHoursEntryStation(
    station_id, station_activity
) -> None:  # horas pico de ingreso a una estación
    ...
    entries = []
    for user_id, event_type, event_time in station_activity[station_id]:
        if event_type == "IN":
            entries.append(event_time)
    ...
    for entry in entries:
        hours.append(
            entry[:2]
        )  # Agregamos a la lista hours la hora de ingreso extrayendo por slicing

    # Usaremos una funcion nativa de Python para contar la frecuencia de cada hora
    hourCounts = Counter(
        hours
    )  # Es un diccionario con la hora de ingreso como clave y la frecuencia como valor
    ...
    for (
        hour,
        count,
    ) in hourCounts.items():  # Recorremos los items del diccionario hourCounts
        if count == maxCount:
...
def peakHoursExitStation(
    station_id, station_activity
) -> None:  # horas pico de salida de una estación
    ...
    exits = []
    for user_id, event_type, event_time in station_activity[
        station_id
    ]:  # Recorremos la lista de eventos de la estación
        if event_type == "OUT":
            exits.append(
                event_time
            )  ## Agregamos a Entries el event_type == 'OUT" el cual contiene la hora de salida.
    ...
    for exit in exits:
        hours.append(
            exit[:2]
        )  # Agregamos a la lista hours la hora de salida extrayendo por slicing
    # print(hours)
...
def mostFrequentOriginStation(
    station_id, user_activity
) -> None:  # estación de origen más frecuente hacia esta estación
    """Funcion que calcula la estación de origen más frecuente hacia una estación específica"""

    origin_counts = (
        {}
    )  # station_id_origen -> cantidad de viajes que llegan a station_id
...
def mostFrequentDestinationStation(
    station_id, user_activity
) -> None:  # estación de destino más frecuente desde esta estación
    """Funcion que calcula la estación de destino más frecuente desde una estación específica"""

    destination_counts = (
        {}
    )  # station_id_destino -> cantidad de viajes que salen desde station_id
    ...
    for destination_station_id in most_frequent_destination_ids:
        print(
            f"{destination_station_id} - Viajes: {destination_counts[destination_station_id]}"
        )
